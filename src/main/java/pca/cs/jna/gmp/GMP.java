...
import static pca.cs.jna.gmp.LibGMP.__gmpz_mod;
...
public final class GMP {
	...
	public static BigInteger mod(BigInteger a, BigInteger b) {//a * b
		return INSTANCE.get().modImpl(a, b);
	}
	private BigInteger modImpl(BigInteger a, BigInteger b) {
		mpz_t peera = getPeer(a, sharedOperands[0]);
		mpz_t peerb = getPeer(b, sharedOperands[1]);
		__gmpz_mod(sharedOperands[2], peera, peerb);
		int requiredSize = a.bitLength() + b.bitLength();
		return new BigInteger(mpzSgn(sharedOperands[2]), mpzExport(sharedOperands[2], requiredSize));
	}
